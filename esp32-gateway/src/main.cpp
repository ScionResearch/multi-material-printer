...
DHCPReservation reservations[] = {
  {"B8:27:EB:48:32:7B", IPAddress(192, 168, 4, 2), "Raspberry Pi"},     // Pi MAC (static IP)
  {"28:6D:CD:A6:D9:F6", IPAddress(192, 168, 4, 3), "Anycubic Printer"} // Printer MAC
};
...
void setup() {
  ...
  Serial.println("Initializing ESP32 Access Point (DHCP disabled - static IPs required)...");

  // Configure Access Point with static IP and DHCP disabled
  WiFi.mode(WIFI_AP);
  ...
  if (ap_success) {
    ...
    Serial.println("Static IP Configuration Required:");
    for (int i = 0; i < num_reservations; i++) {
      Serial.print("  ");
      Serial.print(reservations[i].device_name);
      Serial.print(" (");
      Serial.print(reservations[i].mac_address);
      Serial.print(") must be set to ");
      Serial.println(reservations[i].reserved_ip);
    }
    Serial.println("  DHCP is DISABLED on this network");
    Serial.println();

    Serial.println("Setup Instructions:");
    Serial.println("1. Configure Raspberry Pi with static IP 192.168.4.2/24, gateway 192.168.4.1");
    Serial.println("2. Configure printer with static IP 192.168.4.3/24, gateway 192.168.4.1");
    Serial.println("3. Both devices must connect to WiFi network: PumpedMMP");
    Serial.println();
...
void checkConnectedDevices() {
  ...
  for (int i = 0; i < adapter_sta_list.num; i++) {
    ...
    Serial.print(i + 1);
    Serial.print(": ");
    Serial.print(ip4addr_ntoa((const ip4_addr_t*)&(station.ip)));
    Serial.print(" (MAC: ");
    ...
    for (int r = 0; r < num_reservations; r++) {
      if (reservations[r].mac_address == mac_str) {
        ...
        String current_ip = String(ip4addr_ntoa((const ip4_addr_t*)&(station.ip)));
